...
# Paramètres spécifiques WGAN-GP
lr = 0.0001           # Taux d'apprentissage(Learning rate) 
LAMBDA_GP = 10        # Coefficient de la pénalité de gradient (Standard = 10)
CRITIC_ITERATIONS = 5 # Le critique travaille 5 fois plus
...
print(f"Configuration WGAN-GP sur : {device}")
...
class Discriminator(nn.Module):
    def __init__(self, channels_img, features_d):
        ...
        self.disc = nn.Sequential(
            ...
            # IMPORTANT : InstanceNorm au lieu de BatchNorm pour WGAN-GP
...
def initialize_weights(model):
    ...
    # 1. Création des images interpolées (mix Vrai/Faux)
    epsilon = torch.rand((BATCH_SIZE, 1, 1, 1)).repeat(1, C, H, W).to(device)
    interpolated_images = real * epsilon + fake * (1 - epsilon)

    # 2. Calcul du score du critique sur ces images
    ...
    # 4. Calcul de la norme du gradient et de la pénalité
...
def evaluate_initial_state():
    ...
    with torch.no_grad():
        for idx, (real, _) in enumerate(train):
            ...
            # Loss WGAN simple (sans pénalité ici car on est à l'état initial)
    ...
    with torch.no_grad():
        ...
        plt.axis("off")
        plt.title("Epoch 0 (État Initial)")
...
print('Entraînement du WGAN-GP sur MNIST...')
...
for epoch in range(epochs):
    ...
    for i in range(num_batches):
        ...
        # ---------------------
        # 1. Entraînement Discriminateur (5 itérations)
        # ---------------------
        for _ in range(CRITIC_ITERATIONS):
            ...
            # Loss Critique = -(Score_Vrai - Score_Faux) + Pénalité
        ...
        # ---------------------
        # 2. Entraînement Générateur (1 itération)
        # ---------------------
        ...
        # Logs
        # On divise par CRITIC_ITERATIONS pour garder une échelle comparable
...
plt.title("Courbes de Loss WGAN-GP MNIST")
...
plt.xlabel("Époques")
plt.ylabel("Loss Wasserstein + GP")
plt.legend()
plt.grid(True)
plt.show()
